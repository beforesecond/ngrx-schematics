...
function createAddInjectionContext(options: ModuleOptions, host: Tree,name:string): AddInjectionContext {
   ...
   // console.log(nameRoot)
    ...
    let appComponentFileName = findFileByName(name + '-state.ts', options.path || '/', host);
    let destinationPath = constructDestinationPath(options);
    let serviceName = classify(`${options.name}State`);
...
export function injectServiceIntoAppComponent(options: ModuleOptions, name: string): Rule {
...
function buildInjectionChanges(context: AddInjectionContext, host: Tree, options: ModuleOptions): Change[] {
    ...
    if (!ctorNode) {
        ...
        constructorChange = createExportStateForInjection(context, nodes, options);
    }
    else {
...
function createExportStateForInjection(context: AddInjectionContext, nodes: ts.Node[], options: ModuleOptions): Change {
    let classNode = nodes.find(n => n.kind === ts.SyntaxKind.SourceFile);

    // console.log(classNode.ps)
    if(nodes&&options){
    ...
    // let listNode = siblings.find(n => n.kind === ts.SyntaxKind.LastAssignment);
    ...
    //console.log(context)
//     let toAdd = `
// export interface RootStoreFor${context.serviceName} {
//     feature: ${context.serviceName}.State;
// }`;
let posintion = classNode.getText().indexOf('/// Add State ///')+('/// Add State ///').length+1
let toAdd = `
    ${context.serviceName.toLowerCase().replace('storestate','')}: ${context.serviceName}.State;
`;
