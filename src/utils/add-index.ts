...
function createAddInjectionContext(options: ModuleOptions, host: Tree,name:string): AddInjectionContext {
   ...
   if(name){
    ...
    let appComponentFileName = findFileByName('index.ts', options.path || '/', host);
...
export function injectIndexIntoAppComponent(options: ModuleOptions, name: string): Rule {
...
function createExportSelectorsForInjection(context: AddInjectionContext, nodes: ts.Node[], options: ModuleOptions): Change {
    ...
    let posintion = classNode.getText().indexOf(`/// export ///`)+(`/// export ///`).length
    //let selectValue = classify(`${options.name}`);
    let toAdd = `
export * from './${options.name}'
     `;
     //console.log(classNode.end)
