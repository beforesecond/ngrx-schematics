...
export function injectSelectorsIntoAppComponent(options: ModuleOptions, name: string): Rule {
...
function createExportSelectorsForInjection(context: AddInjectionContext, nodes: ts.Node[], options: ModuleOptions): Change {
    ...
    let posintion = classNode.getText().indexOf(`(): any => 'error',`)+(`(): any => 'error',`).length+1
    let selectValue = classify(`${options.name}`);
    let toAdd = `
    ${context.serviceName}.select${selectValue}Error,
    `;
