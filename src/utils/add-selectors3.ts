...
export function injectSelectors3IntoAppComponent(options: ModuleOptions, name: string): Rule {
...
function createExportSelectorsForInjection(context: AddInjectionContext, nodes: ts.Node[], options: ModuleOptions): Change {
    ...
    let posintion = classNode.getText().indexOf(`return Error`)+(`return Error`).length
