...
export function injectSelectors2IntoAppComponent(options: ModuleOptions, name: string): Rule {
...
function createExportSelectorsForInjection(context: AddInjectionContext, nodes: ts.Node[], options: ModuleOptions): Change {
    ...
    let posintion = classNode.getText().indexOf(`(Error: string`)+(`(Error: string`).length
    let selectValue = classify(`${options.name}`);
    let toAdd = `
    ,${selectValue}Error : string
    `;
