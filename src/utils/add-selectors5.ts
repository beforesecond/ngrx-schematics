...
export function injectSelectors5IntoAppComponent(options: ModuleOptions, name: string): Rule {
...
function createExportSelectorsForInjection(context: AddInjectionContext, nodes: ts.Node[], options: ModuleOptions): Change {
    ...
    let posintion = classNode.getText().indexOf(`(loading: boolean`)+(`(loading: boolean`).length
    let selectValue = classify(`${options.name}`);
    let toAdd = `
    ,${selectValue}IsLoading : boolean
    `;
   // console.log(classNode.getText())
   // console.log(posintion)
